...
class LoginActivity : AppCompatActivity() ,ViewStateCallback<LoginResponse> {
    ...
    private fun playAnimation() {
        var label = ObjectAnimator.ofFloat(loginBinding.label, View.TRANSLATION_Y, -50f, 0f).setDuration(5000)
        var labeel = ObjectAnimator.ofFloat(loginBinding.labeel, View.TRANSLATION_Y, -50f, 0f).setDuration(5000)

        AnimatorSet().apply {
            playTogether(label, labeel)
        }.start()
    }
}
